...
$(document).ready(function() {
   ...
   $.get('/data/ytrecos-election-de/' + url + '.json', function(data) {
    // console.log(data);
      jsonLocal = data;
      let count = 0;
   ...
   $(document).on('change', '#video-select', function(event) {
      $.get(
      '/data/ytrecos-election-de/' + event.target.value + '.json',
      (data) => {
      ...
      `
         <div class="columns">
            <div class="column has-text-centered">
                  ...
                  <h2 class="">in the playlist on the right for the search "<a class="searched-value" href="https://www.youtube.com/results?search_query=${key}" target="_blank"><span id="selected-key">${key}</span></a>"</h2>
            </div>
         </div>
         `
    );
